  ...
  return (
    <PageTransition>
      <div className="bg-[#FBFBF9] min-h-screen">
        {/* Non-print toolbar */}
        <div className="print:hidden bg-white border-b border-[#E2E8F0] py-3 sticky top-17 z-30 shadow-2xs">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 flex flex-wrap items-center justify-between gap-3">
            <Link
              to="/"
              className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#0F2942] hover:text-[#183B5E] p-1.5"
            >
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-[#0F2942] bg-[#F1F5F9] hover:bg-[#E2E8F0] rounded-md transition-colors cursor-pointer min-h-[38px]"
              >
                <Download className="w-3.5 h-3.5" />
                <span>{downloading ? 'Exporting...' : 'Export Plaintext'}</span>
              </button>

              <button
                onClick={handlePrint}
                className="inline-flex items-center gap-1.5 px-4 py-1.5 text-xs font-semibold text-white bg-[#0F2942] hover:bg-[#1A3E61] rounded-md transition-colors shadow-2xs cursor-pointer min-h-[38px]"
              >
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 print:py-0 print:px-0">
          <motion.div
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.35, ease: 'easeOut' }}
            className="bg-white border border-[#CBD5E1] rounded-lg shadow-sm p-6 sm:p-10 lg:p-12 print:border-none print:shadow-none print:p-0 space-y-8 font-serif-academic text-[#1E293B]"
          >
            ...
            <div className="border-b-2 border-[#0F2942] pb-6 space-y-4 font-sans">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                <div className="flex items-start sm:items-center gap-4 sm:gap-5">
                  <div className="relative w-20 sm:w-24 h-auto aspect-[864/1024] rounded-md overflow-hidden border border-[#CBD5E1] shadow-2xs shrink-0">
                    <img
                  ...
                  <div>
                    <h1 className="font-serif-academic text-2xl sm:text-3xl lg:text-4xl font-bold text-[#0F2942] break-words">
                      {PROFESSOR_INFO.name}
                    </h1>
                    <p className="text-sm sm:text-base text-[#475569] font-medium mt-0.5 break-words">
                    ...
                    <div className="pt-2 flex flex-wrap gap-2 sm:gap-3 text-xs text-[#0F2942] font-mono">
                ...
                <div className="text-xs text-left sm:text-right space-y-1 text-[#475569] border-t sm:border-t-0 pt-3 sm:pt-0 w-full sm:w-auto">
                  <div className="font-semibold text-[#0F2942] break-all">{PROFESSOR_INFO.email}</div>
                  <div>{PROFESSOR_INFO.phone}</div>
                  <div className="break-words">{PROFESSOR_INFO.officeLocation}</div>
              ...
              <div className="space-y-3 font-sans text-xs">
                {EDUCATION_HISTORY.map((edu: Education, idx: number) => (
                  <div key={idx} className="flex justify-between items-start gap-4">
                    <div>
                      <div className="font-bold text-[#0F2942] text-sm break-words">
                        {edu.degree} in {edu.discipline}
                      </div>
                      <div className="text-[#475569] break-words">
                        {edu.institution}, {edu.universityOrBoard}
                      </div>
                      {edu.focusArea && (
                        <div className="text-[#64748B] text-[11px] mt-0.5 break-words">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-3 font-sans text-xs">
                {PROFESSIONAL_ROLES.map((item: AcademicRole, idx: number) => (
                  <div key={idx} className="flex justify-between items-start gap-4">
                    <div>
                      <div className="font-bold text-[#0F2942] break-words">
                        {item.role}
                      </div>
                      <div className="text-[#475569] break-words">
            ...
            <div className="space-y-2">
              ...
              <div className="font-sans text-xs p-3.5 bg-[#F8FAFC] border border-[#E2E8F0] rounded-md space-y-1">
                <div className="font-bold text-[#0F2942] text-sm break-words">
                ...
                <div className="text-[#64748B] font-mono text-[11px] break-words">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-3 font-sans text-xs">
                {PUBLICATIONS.map((pub: Publication, idx: number) => (
                  <div key={pub.id} className="leading-relaxed break-words">
                    ...
                    {pub.doi && <span className="font-mono text-[#64748B] text-[11px] break-all">DOI: {pub.doi}</span>}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2 font-sans text-xs">
                {DEPARTMENTAL_COMMITTEES.map((role, idx) => (
                  <div key={idx} className="flex justify-between items-start gap-4">
                    <div>
                      <span className="font-bold text-[#0F2942] break-words">{role.role}</span>
                      <span className="text-[#64748B] break-words"> — {role.scope}</span>
                      <p className="text-[#475569] text-[11px] mt-0.5 break-words">{role.description}</p>
                    </div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 font-sans text-xs">
                {COURSES.map((course: Course) => (
                  <div key={course.code} className="p-2.5 bg-[#F8FAFC] border border-[#E2E8F0] rounded-xs">
                    <span className="font-mono font-bold text-[#0F2942]">{course.code}</span>: <span className="font-medium">{course.title}</span>
                    <div className="text-[11px] text-[#64748B] mt-0.5">{course.semester}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="pt-8 border-t border-[#CBD5E1] font-sans text-xs flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3 text-[#64748B]">
              <div>
